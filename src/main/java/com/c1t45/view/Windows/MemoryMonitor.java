...
//                 long timestamp = System.currentTimeMillis() - startTime; // Current time in milliseconds

//                 // Get heap memory usage
//                 long heapMemoryUsed = memoryMXBean.getHeapMemoryUsage().getUsed();
//                 long heapMemoryUsedMB = heapMemoryUsed / (1024 * 1024);
//                 heapMemoryLabel.setText("Heap Memory Used: " + format.format(heapMemoryUsedMB) + " MB");
...
//                 nonHeapMemoryLabel.setText("Non-Heap Memory Used: " + format.format(nonHeapMemoryUsedMB) + " MB");

//                 maxValue = (double) Math.max(maxValue, Math.max(nonHeapMemoryUsedMB, heapMemoryUsedMB));

//                 // Update series data
//                 heapSeries.getData().add(new XYChart.Data<>(timestamp, heapMemoryUsedMB));
//                 nonHeapSeries.getData().add(new XYChart.Data<>(timestamp, nonHeapMemoryUsedMB));
...
//             private void removeOldData(XYChart.Series<Number, Number> series, double threshold) {
//                 // Iterate through data points and remove those older than the threshold
//                 series.getData().removeIf(data -> data.getXValue().doubleValue() < threshold);
//             }
//         };
//         timer.start();
